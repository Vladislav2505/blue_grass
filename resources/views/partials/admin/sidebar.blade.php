        ...
        <nav class="flex flex-col gap-5 justify-center">
            @foreach(config('menu.admin') as $url => $point)
                @if(is_array($point) && array_key_exists('items', $point))
                    <div>
                        <div class="{{Str::contains($currentUrl, $url) ? 'bg-[#495270]' : 'bg-[#3c486c] hover:bg-[#495270]'}} p-2 rounded-t-[5px] transition-colors">
                    ...
                    <div>
                        <ul class="space-y-3">
                            <li class="hover:bg-[#495270] rounded-[5px] transition-colors">
                                <a href="{{$url}}" class="flex items-center w-full p-2 gap-2">
                                    <img src="{{Vite::asset("resources/images/admin/menu/$url.svg")}}" alt="{{$url}}"/>
                                    {{$point}}
                                </a>
                            </li>
